  ...
  return (
    <div className="p-4 space-y-4">
        ...
        <div className="flex items-center justify-between gap-3 border-b border-slate-700 bg-slate-950/80 px-4 py-3">
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-1">
              Parametric 3D workspace Â· Units: mm
            </p>
          </div>
        ...
        <div className="space-y-4">
          ...
          {category === "Chair" && (
            <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
              ...
              <p className="text-xs text-slate-500">
                Seat, back and all four legs have independent W Ã— H Ã— D values.
              </p>
            </div>
          )}
          ...
          {(category === "Door" || category === "Doors") && (
            <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
              ...
              <p className="text-xs text-slate-500">
                The door panel has its own Width Ã— Height Ã— Thickness values.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="rounded-xl bg-white/5 border border-white/10 p-4 text-xs text-slate-400">
        ...
        <p>â€¢ Click and hold a panel, door, drawer or shelf to resize</p>
        <p>â€¢ Drag normally for width and height; hold Shift for depth</p>
        <p>â€¢ Drag empty space to rotate</p>
        <p>â€¢ Scroll to zoom</p>
        <p>â€¢ Right-click/secondary drag to pan</p>
        <p>â€¢ Click a door or drawer to open/close that exact part</p>
        <p>â€¢ Click a panel/shelf/door/drawer to show its W Ã— H Ã— D</p>
      </div>
    </div>
  );
}
...
function CADKernelStatus({ config }: { config: ThreeDConfig }) {
  ...
  useEffect(() => {
    let active = true;
    setStatus("loading");
    setMessage("Loading OpenCascade.js WebAssembly CAD kernelâ€¦");
  ...
  useEffect(() => {
    ...
    try {
      ...
      setMessage(
          ...
          ? `B-Rep validation OK: ${Math.round(config.width)} Ã— ${Math.round(config.height)} Ã— ${Math.round(config.depth)} mm.`
...
function DetailBox({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-gray-50 rounded-xl p-3">
      ...
      <p className="font-semibold mt-1 text-gray-800">{value || "â€”"}</p>
    </div>
  );
}
